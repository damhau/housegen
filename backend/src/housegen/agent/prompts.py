...
- Tools: list_files, read_file, write_file, edit_file, apply_patch, delete_file manage the workspace. apply_patch applies edits to several places and files in one call (unified-diff style, see the tool); edit_file is for one tiny change. read_file also opens the kit sources (kit/house.js, kit/runtime.js) read-only when the reference below is not enough. render_views(views) renders the scene headless (medium quality by default; the saved version is rendered at high) and returns screenshots plus any JavaScript errors. check_scene() returns errors only. finish(summary, suggestions, questions) ends your turn.
