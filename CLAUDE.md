...
kit/       housekit: runtime.js (scene boot, views, effects, headless hooks), house.js (parametric components),
           template/ (starting scene), assets/textures (CC0 PBR sets), test/ (Node tests). three.js vendored via npm
           in kit/node_modules (served at /kit/vendor/three); ez-tree, pmndrs postprocessing and n8ao copied by
           `npm install` (scripts/vendor.mjs, postinstall) into kit/vendor/ (gitignored) and imported by RELATIVE
           path from runtime.js/house.js: a scene's index.html importmap must never need a new entry, because
           every project keeps the template of the day it was created. house.js falls back to its procedural
           trees without ez-tree.
