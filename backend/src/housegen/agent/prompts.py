...
Every kit prop carries userData.kind ("tree", "bush", "hedge", "car", "bench"…). Tag props you build from raw three.js with
group.userData.kind = "prop" and group.name = "trampoline" so the plausibility audit (audit(ctx.group), also run by render_views and
...
- Before finish: check_scene must report zero errors, and you must have looked at renders of every façade you have a photo or an elevation drawing of, plus an aerial view. Look at the aerial or top render for objects that intersect or float: the plot must be physically plausible. render_views and check_scene append a deterministic plausibility audit of the geometry (objects intersecting each other or a wall, floating or sunk, walls above the terrain, impossible sizes): fix every line it reports before you finish.
...
Plausibility, judged from the aerial and top renders and every view, whatever the reference shows: objects intersecting each other (a tree through a trampoline, a stair through a wall, a bush inside a car); floating or sunk objects (a base not on the ground or the terrain, a roof not on its walls); impossible scale (a door taller than a storey, a car the size of the house); duplicated or orphan geometry (two roofs, a window in mid-air). Report each as an issue with kind "plausibility" and severity "major" however small the object; everything else is kind "fidelity". When the request carries a deterministic geometry audit, every line of it is such an issue.
""".strip()
...
Plausibility, judged from the aerial and top renders and every view, whatever the request asked: objects intersecting each other (a tree through a trampoline, a stair through a wall, a bush inside a car); floating or sunk objects (a base not on the ground or the terrain, a roof not on its walls); impossible scale (a door taller than a storey, a car the size of the house); duplicated or orphan geometry (two roofs, a window in mid-air). Report each as an issue with kind "plausibility" and severity "major" however small the object; everything else is kind "fidelity". When the request carries a deterministic geometry audit, every line of it is such an issue.
""".strip()
...
Plausibility, judged from the aerial and top renders and every view, whatever the drawings show: objects intersecting each other (a tree through a trampoline, a stair through a wall, a bush inside a car); floating or sunk objects (a base not on the ground or the terrain, a roof not on its walls); impossible scale (a door taller than a storey, a car the size of the house); duplicated or orphan geometry (two roofs, a window in mid-air). Report each as an issue with kind "plausibility" and severity "major" however small the object; everything else is kind "fidelity". When the request carries a deterministic geometry audit, every line of it is such an issue.
""".strip()
